# Filename: ReasonSkills.py
# Date: 2/21/26
# Author: Aoi | shadowsnowwolf 
# Reason (Mind and Spirit) skill list

class ReasonSkills:
    S_plus_skills = (
"Sage", 
"Distortion Reaper",
"Psychology + 5",
"Manifestation + 5",
"Self Belief + 5",
"Affirmation Strength Lvl 3",
"Brainwash Resistance + 5",
"Red Pill + 5"
)
    
    S_skills = (
"Distortion Destroyer",
"Stoic Philosopher", 
"Atomic Habiter",
"Scarcity Breaker",
"Oneitis Cure",
"Flaw Breaker + 3",
"Self Belief +  4",
"Abundance Mindset + 5",
"Affirmation Strength Lvl 2",
"Brainwash Resistance + 4"
)
    
    A_plus_skills = (
"Habit Grandmaster",
"Auditory Manifestation",
"Stoicism + 3",
"Self Awareness + 3",
"Psychology + 4",
"Manifestation + 4",
"Abundance Mindset + 4",
"Brainwash Resistance + 3",
"Red Pill + 4",
"Reason Prowess Lvl. 5"
)
    
    A_skills = (
"Distortion Slayer",
"Monk",
"Stoic"
"Visual Manifestation",
"Healthy Habits + 3",
"Self Belief + 3",
"Affirmation Strength Lvl 1"
)
    
    B_plus_skills = (
"Protein Master",
"Habit Scholar",
"Manifestation",
"Healthy Sleep Schedule",
"Self Improver",
"Healthy Habits + 2",
"Self Awareness + 2",
"Flaw Breaker + 2",
"Psychology + 3",
"Manifestation + 3",
"Abundance Mindset + 3",
"Reason Prowess Lvl. 4" 
)
    
    B_skills = (
"Healthy Eater",
"Stoicism + 2",
"Flaw Breaker + 1",
"Self Awareness + 1",
"Brainwash Resistance + 2",
"Red Pill + 3"
)
    
    C_plus_skills = (
"Habit Apprentice",
"Healthy Habits + 1",
"Stoicism + 1",
"Novice Stoic",
"Self Belief + 2",
"Red Pill + 2",
"Reason Prowess Lvl. 3" 
)
    
    C_skills = (
"Bed Maker",
"Meditate",
"Abundance Mindset + 2",
"Psychology + 2",
"Manifestation + 2",
"Brainwash Resistance + 1"
)
    
    D_plus_skills = (
"Journal",
"Protein Drinker", 
"Self Belief + 1",
"Red Pill + 1",
"Reason Prowess Lvl. 2"
)
    
    D_skills = ("Abundance Mindset + 1")
    
    E_plus_skills = (
"Reason Prowess Lvl. 1",
"Psychology + 1",
"Manifestation + 1"
)
    
skill_category = "Reason (Mind and Spirit)"

# Displays the full list of E+ skills
def display_e_plus_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("E+:")
    for skill in ReasonSkills.E_plus_skills:
        print(skill)

# Displays the full list of D skills
def display_d_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("D:")
    print(ReasonSkills.D_skills)

# Displays the full list of D+ skills
def display_d_plus_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("D+:")
    for skill in ReasonSkills.D_plus_skills:
        print(skill)

# Displays the full list of C skills
def display_c_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("C:")
    for skill in ReasonSkills.C_skills:
        print(skill)

# Displays the full list of C+ skills
def display_c_plus_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("C+:")
    for skill in ReasonSkills.C_plus_skills:
        print(skill)

# Displays the full list of B skills
def display_b_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("B:")
    for skill in ReasonSkills.B_skills:
        print(skill)

# Displays the full list of B+ skills
def display_b_plus_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("B+:")
    for skill in ReasonSkills.B_plus_skills:
        print(skill)

# Displays the full list of A skills
def display_a_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("A:")
    for skill in ReasonSkills.A_skills:
        print(skill)

# Displays the full list of A+ skills
def display_a_plus_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("A+:")
    for skill in ReasonSkills.A_plus_skills:
        print(skill)

# Displays the full list of S skills
def display_s_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("S:")
    for skill in ReasonSkills.S_skills:
        print(skill)

# Displays the full list of S+ skills
def display_s_plus_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    print("S+:")
    for skill in ReasonSkills.S_plus_skills:
        print(skill)

# Displays the full list of all skills corresponding to each skill letter
def display_all_reason_skills(show_header=False):
    if show_header:
        print(f"{skill_category} skills:")
    display_e_plus_skills()
    print()
    display_d_skills()
    print()
    display_d_plus_skills()
    print()
    display_c_skills()
    print()
    display_c_plus_skills()
    print()
    display_b_skills()
    print()
    display_b_plus_skills()
    print()
    display_a_skills()
    print()
    display_a_plus_skills()
    print()
    display_s_skills()
    print()
    display_s_plus_skills()
